...
export class dealParser extends Parser {
	...
	public expr(_p?: number): ExprContext {
		...
		this.enterRecursionRule(_localctx, 2, dealParser.RULE_expr, _p);
		let _la: number;
		try {
			let _alt: number;
			this.enterOuterAlt(_localctx, 1);
			...
			while (_alt !== 2 && _alt !== ATN.INVALID_ALT_NUMBER) {
				if (_alt === 1) {
					...
					case 1:
						...
						if (!(_la === dealParser.T__0 || _la === dealParser.T__1)) {
					...
					case 2:
						...
						this.state = 19;
						_la = this._input.LA(1);
						if (!(_la === dealParser.T__2 || _la === dealParser.T__3)) {
						this._errHandler.recoverInline(this);
						} else {
							if (this._input.LA(1) === Token.EOF) {
								this.matchedEOF = true;
...
export class ExprContext extends ParserRuleContext {
	...
	public INT(): TerminalNode | undefined { return this.tryGetToken(dealParser.INT, 0); }
	constructor(parent: ParserRuleContext | undefined, invokingState: number) {
		super(parent, invokingState);
	}
	// @Override
